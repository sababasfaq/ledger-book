...
export default function GeneralLedger() {
  ...
  return (
    <article aria-labelledby="general-ledger-title">
      <header className="flex items-center justify-between mb-4">
        <h1 id="general-ledger-title" className="text-xl font-semibold">General Ledger</h1>
        <div className="flex gap-2">
          <button 
            onClick={() => exportToPDF("General Ledger", withBalance)}
          ...
          <button 
            onClick={() => exportToExcel("General Ledger", withBalance)}
            className="flex items-center gap-1 px-2 py-1 text-xs border border-slate-300 rounded hover:bg-slate-50 transition"
      ...
      <section aria-label="Filters" className="bg-slate-50 p-3 rounded-lg border mb-4">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[200px]">
            <label htmlFor="ledger-search" className="block text-xs font-medium text-slate-500 mb-1">Search</label>
          ...
          <div>
            <label htmlFor="from-date" className="block text-xs font-medium text-slate-500 mb-1">From Date</label>
            <input 
              id="from-date"
              type="date"
              value={fromDate}
          ...
          <div>
            <label htmlFor="to-date" className="block text-xs font-medium text-slate-500 mb-1">To Date</label>
            <input 
              id="to-date"
              type="date"
              value={toDate}
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="add-modal-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b">
              <h2 id="add-modal-title" className="text-lg font-semibold">Add General Ledger Entry</h2>
        ...
        <nav aria-label="Table pagination" className="flex items-center justify-between px-4 py-3 bg-slate-50 border-t text-sm">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <label htmlFor="rows-per-page" className="text-slate-500">Rows:</label>
              <select id="rows-per-page" className="border rounded px-1 py-1 bg-white outline-none" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }}>
