...
export default function UnofficialLedger() {
  const { user } = useAuth();
  ...
  const onSave = async () => {
    try {
      ...
      await api.updateRow("unofficial", editingId, patch);
      setEditingId(null);
      await load();
    } catch (e) {
  ...
  const onDelete = async (id) => {
    ...
    try {
      await api.deleteRow("unofficial", id);
      await load();
    } catch (e) {
  ...
  return (
    <article aria-labelledby="unofficial-ledger-title">
      <header className="flex items-center justify-between mb-4">
        <h1 id="unofficial-ledger-title" className="text-xl font-semibold">Unofficial Ledger</h1>
        <div className="flex gap-2">
          <button 
            onClick={() => exportToPDF("Unofficial Ledger", withBalance)}
          ...
          <button 
            onClick={() => exportToExcel("Unofficial Ledger", withBalance)}
            className="flex items-center gap-1 px-2 py-1 text-xs border border-slate-300 rounded hover:bg-slate-50 transition"
      ...
      <section aria-label="Filters" className="bg-slate-50 p-3 rounded-lg border mb-4">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[200px]">
            <label htmlFor="unofficial-search" className="block text-xs font-medium text-slate-500 mb-1">Search</label>
            <div className="relative">
              <Search size={16} className="absolute left-2.5 top-2.5 text-slate-400" aria-hidden="true" />
              <input 
                id="unofficial-search"
                type="text"
          ...
          <div>
            <label htmlFor="unofficial-from-date" className="block text-xs font-medium text-slate-500 mb-1">From Date</label>
            <input 
              id="unofficial-from-date"
              type="date"
              value={fromDate}
          ...
          <div>
            <label htmlFor="unofficial-to-date" className="block text-xs font-medium text-slate-500 mb-1">To Date</label>
            <input 
              id="unofficial-to-date"
              type="date"
              value={toDate}
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="add-modal-title" className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b">
              <h2 id="add-modal-title" className="text-lg font-semibold">Add Unofficial Ledger Entry</h2>
        ...
        <nav aria-label="Table pagination" className="flex items-center justify-between px-4 py-3 bg-slate-50 border-t text-sm">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <label htmlFor="unofficial-rows-per-page" className="text-slate-500">Rows:</label>
              <select id="unofficial-rows-per-page" className="border rounded px-1 py-1 bg-white outline-none" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }}>
